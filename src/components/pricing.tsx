...
export function PricingSection() {
  ...
  return (
    <section id="pricing" className="relative z-10 bg-transparent py-20 md:py-28 px-4 sm:px-6 md:px-8 overflow-hidden" style={{ perspective: "1200px" }}>
      ...
      <div className="absolute top-1/4 left-[10%] -z-10 h-72 w-72 rounded-full bg-rose-500/10 blur-[120px] pointer-events-none animate-orbFloat" />
      <div className="absolute bottom-1/4 right-[10%] -z-10 h-80 w-80 rounded-full bg-indigo-500/10 blur-[120px] pointer-events-none animate-orbFloat" style={{ animationDelay: "5s" }} />

      <div ref={sectionRef} className="mx-auto max-w-7xl">
        <div className="text-center max-w-2xl mx-auto mb-14 md:mb-20 scroll-reveal">
          <p className="text-[#99F54E] text-xs sm:text-sm font-semibold uppercase tracking-wider mb-2">Pricing</p>
            ...
            return (
              <div
                ...
                className={`relative flex flex-col justify-between rounded-3xl p-6 sm:p-8 transition-all duration-300 ${
                  plan.popular
                    ? "bg-gradient-to-b from-purple-500/10 via-purple-500/[0.02] to-transparent border border-purple-500/60 shadow-2xl shadow-purple-500/10 hover:border-purple-500"
                    : "synapse-glass"
                }`}
              >
                {plan.popular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 rounded-full bg-rose-500 px-4 py-1 text-[11px] font-bold text-white uppercase tracking-wider shadow-lg">
                    Most Popular
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-white/90 mb-4 font-semibold leading-normal">{plan.note}</p>
                  <p className="text-xs sm:text-sm text-white/80 leading-relaxed mb-6 font-normal">{plan.description}</p>

                  <div className="space-y-3 border-t border-white/15 pt-6">
                    {plan.features.map((feat, fIdx) => (
                      <div key={fIdx} className="flex items-start gap-2.5 text-xs sm:text-sm text-white font-medium">
                        <Check className="h-4 w-4 text-[#99F54E] shrink-0 mt-0.5" strokeWidth={2.5} />
                        <span>{feat}</span>
                      </div>
                    ))}
                  </div>
